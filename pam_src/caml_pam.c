...
CAMLprim value caml_pam(value k_value, value keep_value, value dist_value)
{
  ...
  char *keep = Bytes_val(keep_value);
  double *dist = Caml_ba_data_val(dist_value);
  gsl_matrix_view m;
  ...
  nrow = Caml_ba_array_val(dist_value)->dim[0];
  ncol = Caml_ba_array_val(dist_value)->dim[1];
  ...
  res_ptr = Caml_ba_data_val(res_bigarr);
  medoids_ptr = medoids;
  for (i = 0; i < k; ++i) {
    *res_ptr++ = *medoids_ptr++;
  }
