...
let test_parsed_equality _ =
  let p1, p2 = Newick.of_string unlabeled, Newick.of_string labeled in
  "labeled != unlabled" @? gtree_equal p1 p2

let test_string_representation _ =
  let p1 = Newick.of_string higher_labeled in
  let s1 = Newick.to_string ~with_edge_labels:true p1 in
  let p2 = Newick.of_string s1 in
  let s2 = Newick.to_string ~with_edge_labels:true p2 in
